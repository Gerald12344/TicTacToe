...
console.log(
    "%c[Harv-Script]" + "%c - Running Harv-Script Developmet v1.0.1!",
    "color:#00B5E2",
    "color:white"
);
...
function element(type, text, id, parent, className) {
...
function booleanComponentRendering(boolean, component, variables) {
    ...
    function updateSettingsCycle() {
        ...
        logError(
            "Failed, incorrect use of usePortal, the first input must be a hook, that is a boolean!"
        );
    }
...
function ReactfulElement(ElementType, reactVar, parent, UUID, ClassName) {
    if (UUID === undefined) {
        UUID = uuidv4();
    }
    let reactfulVars = [];
    ...
    if (Array.isArray(reactVar)) {
        ...
        Element = element(ElementType, text, UUID, parent, ClassName.aInternal);
...
function generateTemplateWithElement(
    predefinedVars,
    defaultValueArray,
    valueTemplate
) {
...
function generateModernTemplate(template) {
    ...
    let TemplateOut = generateTemplateWithElement(
        variables,
        defaultValues,
        templateDone
    );
...
function useUpdate(inputFunc, arrayToWatch) {
    ...
    arrayToWatch.forEach((e) => {
        /*Now verify that e actually is a reactful variable*/ if (
            typeof e === "object" &&
            !(e === null)
        ) {
            /*Now check if has the right properties*/ if (
        ...
        logError(
            "Failed, incorrect use of hooks, useUpdate must use a array of useHook variables!"
        );
    });
    return killFunc;
}
function useHook(defaultVal) {
    /*lets make a reactive variable*/ let mainReactfulVar =
        ReactFulVariableMaker(defaultVal);
    function update(val) {
...
function Debugger(val) {
    window.onerror = (error, url, line) => {
        if (val) {
            console.log(
                "%c[Harv-Script]" +
                    "%c - Error occurred that could not be dismissed!\n" +
                    "%c" +
                    error,
                "color:#00B5E2",
                "color:white",
            ...
            makeAjaxRequest(
                window.location.origin + "/api/error",
                "POST",
                objectTosend
            );
            return (() => {
...
function board(parentIn, props, player, callback) {
    return (() => {
                ...
                (() => {
                    ...
                    ((parent) => {
                        ...
                        (() => {
                            ...
                            ElementWeWant.Element.addEventListener(
                                "click",
                                function (e) {
                                    if (callback(0) === false) {
                                        return undefined;
...
function tixTaxToe() {
    ...
    function validateMove(num) {
        ...
        let itemGetting = arrayTosend[num];
    ...
    if (typeof useUpdateArray === "undefined") {
        ...
        logError(
            "Failed, incorrect use of hooks, useUpdate can only be used inside a hookFunction!"
        );
    } else {
        useUpdateArray.push(
            useUpdate(() => {
...
function WarningComponent(Mainparent) {
    return (() => {
        /* Basic Component Renderer */ let parent = Mainparent;
        let components = [];
        /* Function To Remove Component*/ (() => {
